{{/*
Expand the name of the chart.
*/}}
{{- define "webapp.name" -}}
{{- default .Chart.Name .Values.nameOverride | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Create a default fully qualified app name.
We truncate at 63 chars because some Kubernetes name fields are limited to this (by the DNS naming spec).
If release name contains chart name it will be used as a full name.
*/}}
{{- define "webapp.fullname" -}}
{{- if .Values.fullnameOverride }}
{{- .Values.fullnameOverride | trunc 63 | trimSuffix "-" }}
{{- else }}
{{- $name := default .Chart.Name .Values.nameOverride }}
{{- if contains $name .Release.Name }}
{{- .Release.Name | trunc 63 | trimSuffix "-" }}
{{- else }}
{{- printf "%s-%s" .Release.Name $name | trunc 63 | trimSuffix "-" }}
{{- end }}
{{- end }}
{{- end }}

{{/*
Create chart name and version as used by the chart label.
*/}}
{{- define "webapp.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" }}
{{- end }}

{{- define "web.fullname" -}}
{{- $name := include "webapp.fullname" . -}}
{{- $role := required "A valid .Values.role entry required!" .Values.web.role -}}
{{- printf "%s-%s" $name $role | trunc 63 | trimSuffix "-" -}}
{{- end -}}


{{- define "cron.fullname" -}}
{{- $name := include "webapp.fullname" . -}}
{{- $role := required "A valid .Values.role entry required!" .Values.cron.role -}}
{{- printf "%s-%s" $name $role | trunc 63 | trimSuffix "-" -}}
{{- end -}}



{{/*
Common labels
*/}}
{{- define "webapp.labels" -}}
app.kubernetes.io/part-of: {{ include "webapp.fullname" . }}
helm.sh/chart: {{ include "webapp.chart" . }}
{{- if .Chart.AppVersion }}
app.kubernetes.io/version: {{ .Chart.AppVersion | quote }}
{{- end }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
{{- end }}

{{- define "web.labels" -}}
{{ include "web.selectorLabels" . }}
{{ include "webapp.labels" . }}
{{- end -}}

{{- define "web.selectorLabels" -}}
app.kubernetes.io/name: {{ include "web.fullname" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end -}}

{{- define "cron.labels" -}}
{{ include "cron.selectorLabels" . }}
{{ include "webapp.labels" . }}
{{- end -}}

{{- define "cron.selectorLabels" -}}
app.kubernetes.io/name: {{ include "cron.fullname" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end -}}

{{/*
Create the name of the service account to use
*/}}
{{- define "webapp.serviceAccountName" -}}
{{- if .Values.serviceAccount.create }}
{{- default (include "webapp.fullname" .) .Values.serviceAccount.name }}
{{- else }}
{{- default "default" .Values.serviceAccount.name }}
{{- end }}
{{- end }}


{{- define "ingress.tls.secretName" -}}
{{ include "webapp.fullname" . }}-ingress-tls
{{- end -}}

